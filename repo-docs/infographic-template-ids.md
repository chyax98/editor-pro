# Infographic `template-id` 列表（AntV Infographic）

来源：`node_modules/@antv/infographic/esm/templates/*`（当前依赖版本见 `package.json`）

说明：
- `template-id` 必须写在 ` ```infographic` 代码块第一行：`infographic <template-id>`
- 本表用于给 AI/人类“选模板”时做参考（B）
- 统计方式：从模板源码中提取/推导（包含 `hierarchy-tree-*` 这类程序化生成的模板）
