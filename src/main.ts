...
import {
	...
	createOverdueHighlighter,
	registerInfographicRenderer,
...
export default class EditorProPlugin extends Plugin {
	...
	async onload() {
		...
		this.addCommand({
			...
			editorCallback: (editor: Editor, ctx) => {
				if (ctx.file) {
					void this.remoteImageScheduler.scanAndSchedule(
						editor,
						ctx.file,
					);
				}
		...
		if (this.settings.enableSmartToggle) {
			this.addCommand({
				...
				editorCallback: (editor: Editor) =>
					this.safeExecute(
						() =>
							smartToggle(editor, { marker: "**", name: "Bold" }),
						"加粗失败",
					),
			});
			this.addCommand({
			...
			this.addCommand({
				...
				editorCallback: (editor: Editor) =>
					this.safeExecute(
						() =>
							smartToggle(editor, { marker: "`", name: "Code" }),
						"行内代码失败",
					),
			});
		}
		...
		this.addCommand({
			...
			editorCallback: (editor: Editor) =>
				changeCalloutType(editor, this.app),
		});
		...
		if (this.settings.enableZoom) {
			this.addCommand({
				...
				editorCallback: (editor) =>
					zoomCurrentHeading(this.app, editor),
			});
			this.addCommand({
				id: "zoom-list-block",
				name: "聚焦当前列表块 (Zoom list block)",
				editorCallback: (editor) =>
					zoomCurrentListBlock(this.app, editor),
			});
		}
		...
		if (this.settings.enableRandomGenerator) {
			...
			this.addCommand({
				...
				name: "插入随机整数…",
				editorCallback: (editor) =>
					insertRandomIntPrompt(this.app, editor),
			});
			this.addCommand({
				id: "dice-roll",
				name: "掷骰子…",
				editorCallback: (editor) =>
					insertDiceRollPrompt(this.app, editor),
			});
		}
			...
			this.registerEvent(
				this.app.workspace.on(
					...
					(menu: Menu, editor: Editor) => {
						...
						if (selection) {
							...
							menu.addItem((item) => {
								item.setTitle("包裹为 Callout 提示块")
									.setIcon("info")
									.onClick(() => {
										new CalloutTypePicker(
											this.app,
											(type) =>
												wrapWithCallout(editor, {
													type,
												}),
										).open();
									});
						...
						if (
							this.settings.enableSearchInSelection &&
							selection
						) {
							menu.addItem((item) => {
								item.setTitle("选区查找替换…")
									.setIcon("search")
									.onClick(() => {
										new SearchInSelectionModal(
											this.app,
											editor,
										).open();
									});
						...
						if (isInTable) {
							...
							menu.addItem((item) => {
								item.setTitle("在下方插入行")
									.setIcon("table")
									.onClick(() => {
										const allLines = editor
											.getValue()
											.split("\n");
										const newLines = insertRow(
											allLines,
		...
		if (this.settings.enableSaveCleaner) {
			new SaveCleaner({
				app: this.app,
				enabled: () => this.settings.enableSaveCleaner,
			}).register(this);
		}
		...
		this.registerDomEvent(document, "keydown", (evt: KeyboardEvent) => {
			...
			if (view) {
				if (
					!this.settings.enableSmartTyping &&
					!this.settings.enableEditorNavigation &&
					!this.settings.enableOutliner
				)
					return;
		...
		this.registerEvent(
			this.app.workspace.on(
				"editor-paste",
				(evt: ClipboardEvent, editor: Editor, info) => {
					void this.safeExecuteAsync(async () => {
						// A) 图片粘贴：重命名归档
						if (this.settings.enableSmartImagePaste) {
							const handled = await handleSmartImagePaste(
								this.app,
								editor,
								{ file: info.file },
								evt,
						...
						if (this.settings.enableSmartPasteUrl) {
							const handled = smartPasteUrlIntoSelection(
								editor,
								text,
							);
							if (handled) {
						...
						// C) URL 自动标题（无选区）
						if (
							this.settings.enableSmartLinkTitle &&
							...
							const fromHtml = html
								? extractTitleFromClipboardHtml(html, url)
								: null;
							const title = fromHtml?.trim()
								? fromHtml.trim()
								: this.settings.enableSmartLinkTitleNetwork
		...
		this.registerEditorExtension(
			createOverdueHighlighter(
				() => this.settings.enableOverdueHighlighter,
			),
		);
		...
		if (this.settings.enableFloatingOutline) {
			this.floatingOutline = new FloatingOutline({
				app: this.app,
				enabled: () => this.settings.enableFloatingOutline,
			});
		...
		if (this.settings.enableStatusBarStats) {
			new StatusBarStats({
				app: this.app,
				enabled: () => this.settings.enableStatusBarStats,
			}).register(this);
		}

		// 14. 文件列表增强（icon/banner + file tree highlight）
		if (this.settings.enableInlineDecorator) {
			this.inlineDecorator = new InlineDecorator({
				app: this.app,
				enabled: () => this.settings.enableInlineDecorator,
			});
		...
		this.addSettingTab(
			new EditorProSettingTab(this.app, this, this.mcpFeature),
		);
	}
	...
	private async openFileTreeHighlightMenu() {
		...
		for (const color of colors) {
			menu.addItem((item) => {
				...
				item.onClick(
					() => void this.setFileTreeHighlight(file.path, color),
				);
			});
		}
	...
	private async setFileTreeHighlight(path: string, color: HighlightColor) {
		...
		await this.savePluginData({
			fileTreeHighlights: this.fileTreeHighlights,
		});
	}
	...
	async loadSettings() {
		...
		this.settings = Object.assign(
			{},
			DEFAULT_SETTINGS,
			(loaded as Partial<EditorProSettings>) ?? {},
		);
	}
	...
	private async savePluginData(patch: {
		...
		const next = {
			...
			fileTreeHighlights:
				patch.fileTreeHighlights ?? base.fileTreeHighlights,
		};
