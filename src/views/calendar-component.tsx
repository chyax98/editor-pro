...
export const CalendarComponent: React.FC<CalendarProps> = ({ app }) => {
    ...
    const handlePrev = () => setCurrentDate(currentDate.clone().subtract(1, 'month'));
    const handleNext = () => setCurrentDate(currentDate.clone().add(1, 'month'));
    ...
    const openDailyNote = async (day: number) => {
        ...
        if (existing) {
            ...
            try {
                const newFile = await app.vault.create(path, `# ${targetDate.format('YYYY-MM-DD')}\n\n`);
