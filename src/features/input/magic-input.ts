...
export function checkMagicDateInput(
	line: string,
	cursorCh: number,
): Replacement | null {
	...
	if (next) {
		...
		return {
			replacement: formatDate(d),
			range: { from: cursorCh - next[0].length, to: cursorCh },
		};
	}
	...
	if (cnSimple) {
		...
		return {
			replacement: formatDate(d),
			range: { from: cursorCh - cnSimple[0].length, to: cursorCh },
		};
	}
	...
	if (cnNext) {
		...
		return {
			replacement: formatDate(d),
			range: { from: cursorCh - cnNext[0].length, to: cursorCh },
		};
	}
...
export function checkMagicSymbols(
	line: string,
	cursorCh: number,
): Replacement | null {
	if (cursorCh === 0) return null;
	...
	for (const { needle, replacement } of table) {
		...
		return {
			replacement,
			range: { from: cursorCh - needle.length, to: cursorCh },
		};
	}
	return null;
}
...
export function applyReplacementAtCursor(editor: Editor, r: Replacement) {
	...
	editor.replaceRange(
		r.replacement,
		{ line: cursor.line, ch: r.range.from },
		{ line: cursor.line, ch: r.range.to },
	);
}
