...
import {
	...
	generateWeekly,
	generateHTML,
} from "../../utils/markdown-generators";
...
const COMMANDS: SlashCommand[] = [
	...
	{
		id: "mermaid",
		name: "Mermaid 图表 (Mermaid)",
		aliases: ["mermaid", "mm"],
	},
	{ id: "d2", name: "D2 图表 (D2)", aliases: ["d2"] },
	{
		id: "graph",
		name: "Graph 图表 (DOT/Graphviz)",
		aliases: ["graph", "dot", "graphviz"],
	},
	{
		id: "infographic",
		name: "信息图 (Infographic)",
		aliases: ["infographic", "info", "xx"],
	},
	...
	onTrigger(
		...
		if (trigger) {
			return {
				start: {
					line: cursor.line,
					ch: cursor.ch - trigger.query.length - 1,
				},
				end: cursor,
				query: trigger.query,
			};
			...
			default:
				{
					...
					if (tpl) {
						new TemplateEngine(this.app).insert(
							editor,
							tpl.template,
						);
					}
