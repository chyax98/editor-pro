...
export const HomepageComponent: React.FC<HomepageProps> = ({
    ...
    const loadData = React.useCallback(() => {
        if (settings.homepageShowFolderStats) {
            setFolderStats(getFolderStats(app, trackedFolders, reminderFolders));
        }
        if (settings.homepageShowRecentFiles) {
            setRecentFiles(getRecentFiles(app, settings.homepageRecentFilesCount));
        }
        if (settings.homepageShowReminders) {
            setReminders(getReminders(app, reminderFolders, settings.homepageWeeklyCleanDay));
        }
        if (settings.homepageShowDailyNote) {
            setTodayNoteExists(checkTodayNoteExists(app, settings.homepageDailyNotesFolder));
        }
    ...
    return (
        <div className="homepage-container">
            ...
            {settings.homepageShowGreeting && (
                <div className="homepage-header">
                    ...
                    <div className="homepage-date">
                        ...
                        <span className="homepage-date-weekday">{today.weekday}</span>
                    </div>
                </div>
            )}
            ...
            <div className="homepage-quick-actions">
                ...
                {monthlyPath && (
                    ...
                    >
                        ...
                        <span className="action-hint">{moment().format('MMM')}</span>
                    </button>
                )}
            </div>
            ...
            {settings.homepageShowFolderStats && folderStats.length > 0 && (
                <div className="homepage-section">
                    ...
                    <div className="homepage-folder-flow">
                        {folderStats
                            .filter(fs => trackedFolders.find(tf => tf.path === fs.path)?.showInFlow)
                            .map((fs, idx, arr) => (
                                <React.Fragment key={fs.path}>
                                    ...
                                    >
                                        <span className="folder-icon">{fs.icon}</span>
                                        <span className="folder-name">{fs.name}</span>
                                        <span className="folder-count">{fs.count} 篇</span>
                                        {fs.needsAttention && (
                                            <span className="folder-attention">⚠️</span>
                                        )}
                                    </button>
                                    {idx < arr.length - 1 && (
                                        <span className="folder-flow-arrow">→</span>
                                    )}
                                </React.Fragment>
                            ))}
                    </div>
                    ...
                    <div className="homepage-recent-list">
                        {recentFiles.map(file => (
                            ...
                            >
                                ...
                                <span className="recent-time">{file.mtimeDisplay}</span>
                            </button>
                        ))}
                    </div>
                </div>
            ...
            {/* Pinned Notes */}
            {settings.homepageShowPinnedNotes && settings.homepagePinnedNotes.length > 0 && (
                <div className="homepage-section">
                    <div className="homepage-section-title">📌 置顶笔记</div>
                    <div className="homepage-pinned-list">
                        {settings.homepagePinnedNotes.map(path => {
                            ...
                            return (
                                ...
                                >
                                    <span className="pinned-name">{file.basename}</span>
                                    <span className="pinned-folder">{file.parent?.path || ''}</span>
                                </button>
                            );
                        })}
                    </div>
            ...
            <div className="homepage-footer">
                <button className="homepage-refresh-btn" onClick={onRefresh} title="刷新">
                    🔄
                </button>
            </div>
        </div>
