...
import { FolderStats, RecentFile, ReminderItem, TrackedFolder, ReminderFolder } from './types';
 ...
 * 获取今日日期的显示格式
 */
export function getTodayDisplay(): { date: string; weekday: string; full: string } {
    const now = moment();
 ...
 * 获取最近编辑的文件
 */
export function getRecentFiles(app: App, limit: number, excludeFolders: string[] = []): RecentFile[] {
