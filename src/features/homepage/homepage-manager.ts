...
export class HomepageManager {
	...
	register(): void {
		...
		plugin.registerEvent(
			plugin.app.workspace.on("file-menu", (menu: Menu, file) => {
				...
				menu.addItem((item) => {
					item.setTitle(isPinned ? "从首页取消置顶" : "置顶到首页")
	...
	refreshHomepageViews(): void {
		const leaves =
			this.plugin.app.workspace.getLeavesOfType(HOMEPAGE_VIEW_TYPE);
	...
	cleanup(): void {
		this.plugin.app.workspace
			.getLeavesOfType(HOMEPAGE_VIEW_TYPE)
			.forEach((leaf) => {
				leaf.detach();
			});
	}
