...
export class RemoteImageTaskScheduler {
    ...
    public scanAndSchedule(editor: Editor, file: TFile) {
        ...
        for (const match of matches) {
            ...
            // 检查是否已在队列中
            const exists = this.queue.some(t => t.url === url && t.filePath === file.path);
            if (exists) continue;
     ...
     */
    private async replaceInFile(file: TFile, target: string, replacement: string) {
        // 1. 尝试查找打开了该文件的编辑器
    ...
    private async saveImage(data: ArrayBuffer, ext: string, prefix: string): Promise<string> {
