    ...
    if (selection.length > 0) {
        ...
        if (markedRange) {
            ...
            if (word.length > 0) {
                ...
                tx.selection = {
                    from: { line: cursor.line, ch: wordRange.from },
                    to: { line: cursor.line, ch: wordRange.from + config.marker.length + word.length + config.marker.length }
                };
            } else {
                ...
                tx.selection = {
                    from: { line: cursor.line, ch: cursor.ch + config.marker.length },
                    to: { line: cursor.line, ch: cursor.ch + config.marker.length }
                };
            }
        }
    }
