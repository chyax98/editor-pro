...
export class TagManager {
	...
	async renameTag(oldTag: string, newTag: string) {
		...
		for (const file of files) {
			...
			// 1. Inline tags
			const hasInline = cache.tags?.some(
				(t) => t.tag === oldTag || t.tag.startsWith(oldTag + "/"),
			);
			...
			const hasFrontmatter = frontmatterTags?.some(
				(t) => "#" + t === oldTag || ("#" + t).startsWith(oldTag + "/"),
			);
		...
		new Notice(
			`已重命名标签 ${oldTag} -> ${newTag} (涉及 ${updatedCount} 个文件)`,
		);
	}
				...
				if (fm.tags) {
					...
					const processArray = (arr: string[]) => {
						return arr.map((t) => {
							...
							if (t.startsWith(oldRaw + "/"))
								return t.replace(oldRaw + "/", newRaw + "/");
							return t;
						});
					};
