...
export class VaultGuardianHealthReportModal extends Modal {
	...
	onOpen(): void {
		...
		contentEl.createEl("h2", {
			text: result.isHealthy ? "✅ 目录结构健康" : "⚠️ 发现目录结构问题",
		});
		...
		statsList.createEl("li", {
			text: `根目录数量: ${result.stats.rootFolders}`,
		});
		statsList.createEl("li", {
			text: `总目录数量: ${result.stats.totalFolders}`,
		});
		statsList.createEl("li", {
			text: `最大嵌套深度: ${result.stats.maxDepthFound}`,
		});

		// Violations
		if (result.violations.length > 0) {
			const violationsEl = contentEl.createDiv({
				cls: "vault-guardian-violations",
			});
			violationsEl.createEl("h4", {
				text: `❌ 发现 ${result.violations.length} 个问题`,
			});
