...
export class VaultGuardianManager {
    ...
    register(): void {
        ...
        plugin.registerEvent(
            plugin.app.vault.on('create', (file) => {
                ...
                const allowedRoots = parseAllowedRootFolders(plugin.settings.vaultGuardianAllowedRoots);
                const folderRules = parseFolderRules(plugin.settings.vaultGuardianFolderRules);

                const result = checkCreationAllowed(allowedRoots, folderRules, file.path, true);
    ...
    private runHealthCheckCommand(): void {
        const settings = this.plugin.settings;
        const allowedRoots = parseAllowedRootFolders(settings.vaultGuardianAllowedRoots);
        const folderRules = parseFolderRules(settings.vaultGuardianFolderRules);

        const result = runHealthCheck(this.plugin.app, allowedRoots, folderRules);
    ...
    private runSilentHealthCheck(): void {
        ...
        if (!result.isHealthy) {
            new Notice(`⚠️ Vault Guardian: 发现 ${result.violations.length} 个目录结构问题`, 5000);
        }
    }
