...
export class VaultGuardianWarningModal extends Modal {
	...
	onOpen(): void {
		...
		if (violation.suggestion) {
			const suggestionEl = contentEl.createDiv({
				cls: "vault-guardian-suggestion",
			});
		...
		new Setting(actionsEl)
			...
			.addButton((btn) => {
				if (blockCreation) {
					btn.setButtonText("已阻止").setDisabled(true);
				} else {
					btn.setButtonText("仍然创建")
						.setWarning()
						.onClick(() => {
							this.close();
							this.onProceed();
