...
export class SearchInSelectionModal extends Modal {
	...
	onOpen() {
		...
		new Setting(this.contentEl).setName("查找").addText((text) => {
			text.setPlaceholder("要查找的内容").onChange(
				(v: string) => (this.find = v),
			);
		...
		new Setting(this.contentEl).setName("替换为").addText((text) => {
			text.setPlaceholder("替换成什么").onChange(
				(v: string) => (this.replace = v),
			);
		...
		new Setting(this.contentEl)
			...
			.addToggle((toggle) => {
				toggle
					...
					.onChange((v) => (this.caseSensitive = v));
				toggle.toggleEl.setAttribute("aria-label", "区分大小写");
			});

		new Setting(this.contentEl)
			.addButton((btn) => {
				btn.setButtonText("替换全部")
					.setCta()
					.onClick(() => {
						if (!this.find) {
						...
						if (this.find.length > MAX_SEARCH_LENGTH) {
							new Notice(
								`Editor Pro：查找内容过长，最大支持 ${MAX_SEARCH_LENGTH} 个字符`,
							);
							return;
						}
						...
						const escaped = this.find.replace(
							/[.*+?^${}()|[\]\\]/g,
							"\\$&",
						);
